...
Read-only career timeline for an employee: department transfers, position changes, salary changes, employment status transitions, reporting manager changes, probation end date changes, and intern end date changes.
...
- `field`: optional string, one of `department_id`, `position_id`, `base_salary`, `employment_status`, `probation_end_date`, `intern_end_date`, `manager_id`
...
- `employment_status`: `{ "value": "<full-time|probation|intern|resigned|terminated>" }`
...
- `intern_end_date`: `{ "value": "<YYYY-MM-DD>|null" }`, or `old_value` is bare `null` when the employee had no intern end date before this change.
