...
Business rule: `employee_id` is generated by the backend in `EMP-XXXXX` format and is read-only for clients.
...
- If a client sends `employee_id` on create, the backend ignores it.
